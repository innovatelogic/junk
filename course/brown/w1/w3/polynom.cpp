...
public:
  class IndexProxy
  {
    public:
      ...
      IndexProxy& operator =(T v) 
      {
        //std::cout << "operator = " << poly_.coeffs_.size() << " " << degree_ << std::endl;
         if (poly_.coeffs_.size() < degree_) 
         {
            poly_.coeffs_.resize(degree_ + 1);
            //std::cout << "resize = " << poly_.coeffs_.size() << " " << degree_ << std::endl;
...
public:
  ...
  bool operator ==(const IndexProxy& other) const {
    return coeffs_ == other.poly.coeffs_;
  }
  ...
  int Degree() const 
  {
    std::vector<T> &ref = const_cast<std::vector<T>&>(coeffs_);

    while (ref.size() > 1 && ref.back() == 0) {
      ref.pop_back();
    }
...
template <typename T>
std::ostream& operator <<(std::ostream& out, const typename Polynomial<T>::IndexProxy& p) {
  bool printed = false;
  for (int i = p.poly_.Degree(); i >= 0; --i) {
    if (p.poly_.coeffs_[i] != 0) {
      PrintCoeff(out, i, p.poly_.coeffs_[i], printed);
      printed = true;
    }
  }
  return out;
