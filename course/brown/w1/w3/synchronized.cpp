...
public:
    ...
    struct Access
    {
    Access(T& ref, std::mutex &ref_mutex)
    : ref_to_value(ref)
    , ref_mutex(ref_mutex)
    {
        //std::cerr << "get access" << std::endl;
        ref_mutex.lock();
    }

    Access(const Access& source)
    {
        //std::cerr << "get access copy" << std::endl;
        ref_to_value = source.ref_to_value;
        ref_mutex.lock();
        //source.data_mutex.unlock();
    }
   ...
   struct AccessConst
   {
    AccessConst(const T& ref, std::mutex *guard)
    : ref_to_value(ref)
    , p_mutex(guard)
    {
        p_mutex->lock();
