...
std::string BR(const std::string &in)
{
...
public:
    ...
    std::string ToString() const override { return std::to_string(Evaluate()); }
...
public:
    ...
    int Evaluate() const override { return m_left->Evaluate() + m_right->Evaluate(); }

    std::string ToString() const override { return BR(m_left->ToString()) + "+" +BR(m_right->ToString()); }
