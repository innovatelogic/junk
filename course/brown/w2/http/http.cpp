...
static const std::string POST_CMD = "POST";
static const std::string GET_CMD = "GET";


static const std::string ADD_USER_REQUEST = "/add_user";
static const std::string ADD_COMMENT_REQUEST = "/add_comment";
static const std::string CHECK_CAPTCHA_REQUEST = "/checkcaptcha";
static const std::string USER_COMMENTS_REQUEST = "/user_comments";
static const std::string CAPTCHA_REQUEST = "/captcha";
...
public:
    ...
    HttpResponse& AddHeader(string name, string value)
    {
        ...
        m_values.emplace_back(std::make_pair(std::move(name), std::move(value)));
        return *this;
    }
...
ostream& operator << (ostream& output, const HttpResponse& resp)
{
    ...
    for (auto &contents : resp.m_values){
        output << contents.first << ": " << contents.second << "\n";
    }

    if (!resp.m_content.empty()) {
        output << "Content-Length: " << std::to_string(resp.m_content.size()) << "\n"; 
    }
...
public:
    ...
    {
        if (req.method == POST_CMD)
        {
            if (req.path == ADD_USER_REQUEST) 
            {
            ...
            else if (req.path == ADD_COMMENT_REQUEST) 
            {
            ...
            else if (req.path == CHECK_CAPTCHA_REQUEST) 
            {
        ...
        else if (req.method == GET_CMD)
        {
            if (req.path == USER_COMMENTS_REQUEST) 
            {
            ...
            else if (req.path == CAPTCHA_REQUEST) 
            {
                //os << "HTTP/1.1 200 OK\n" << "Content-Length: 80\n" << "\n"
